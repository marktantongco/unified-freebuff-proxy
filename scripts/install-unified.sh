#!/usr/bin/env bash
# install-unified.sh — complete unified installer for the freebuff-unified +
# unified-owl + opencode ecosystem. Distro-aware (Arch/Omarchy primary,
# Debian/Ubuntu fallback). Omarchy-specific desktop extras live in
# install-omarchy.sh, which delegates here.
#
# Installs:
...
#   6. Owl-agent stack: supervisor wrapper, units, 3-way sync
#   7. Opencode CLI: providers, agents, failover daemon
#   8. Verifies: build check, unit states, endpoint smoke
#
# Usage:
#   ./scripts/install-unified.sh [--yes] [--dry-run] [--no-sudo]
#       [--skip-build] [--verify-only] [--uninstall] [--repo-dir DIR]
#       [--skip-owl] [--skip-opencode]
#
# Flags:
...
#   --skip-owl    skip stage 6 (owl-agent stack)
#   --skip-opencode skip stage 7 (opencode CLI wiring)
#
...
YES=0 DRY=0 NO_SUDO=0 SKIP_BUILD=0 VERIFY_ONLY=0 UNINSTALL=0 REPO_DIR="" SKIP_OWL=0 SKIP_OPENCODE=0
while [ $# -gt 0 ]; do
  case "$1" in
    --yes) YES=1; shift ;;
    --dry-run) DRY=1; shift ;;
...
if [ "$UNINSTALL" -eq 1 ]; then
  step "uninstall: stop + disable units (repos/configs kept; owl-agent untouched)"
...
# ----------------------------------------------------------------------------
# 7. owl-agent stack (supervisor wrapper + 3-way sync)
# ----------------------------------------------------------------------------
step "owl-agent stack"
if [ "$SKIP_OWL" -eq 1 ]; then
  info "skipped (--skip-owl)"
elif [ "$VERIFY_ONLY" -eq 1 ]; then
  info "verify-only: skip owl-agent install"
else
  OWL_SRC="$HOME/workspace/unified-owl"
  if [ ! -d "$OWL_SRC/.git" ]; then warn "no unified-owl checkout at $OWL_SRC (cloned in stage 3)"; fi
  # 3-way sync source -> x1 + root installs (idempotent, md5-guarded).
  # Runs as invoking user with sudo for the two targets; mirrors owl-sync.
  for f in owl_server.py proxy_defense.py chameleon_ai.py forward_proxy.py owl_resilient_mcp.py; do
    [ -f "$OWL_SRC/$f" ] || { warn "missing $f in $OWL_SRC"; continue; }
    for target in /home/x1/.owl-agent /root/.owl-agent; do
      [ -d "$target" ] || { info "no $target; skip"; continue; }
      if [ -f "$target/$f" ] && cmp -s "$OWL_SRC/$f" "$target/$f"; then
        info "$f in sync at $target"
      else
        run sudo install -m 755 -o "$(stat -c '%U' "$target")" -g "$(stat -c '%G' "$target")" "$OWL_SRC/$f" "$target/$f"
        ok "synced $f -> $target"
      fi
    done
  done
  # supervisor wrapper (idempotent: only install when missing/drifted)
  SUP_SRC="$OWL_SRC/scripts/owl-agent-supervisor.sh"
  if [ ! -f "$SUP_SRC" ]; then
    warn "no supervisor template at $SUP_SRC; keeping existing wrapper"
  else
    for target in /home/x1/.owl-agent /root/.owl-agent; do
      [ -d "$target" ] || continue
      if [ -f "$target/owl-agent-supervisor.sh" ] && cmp -s "$SUP_SRC" "$target/owl-agent-supervisor.sh"; then
        info "supervisor in sync at $target"
      else
        run sudo install -m 755 "$SUP_SRC" "$target/owl-agent-supervisor.sh"
        ok "supervisor -> $target"
      fi
    done
  fi
  if [ "$NO_SUDO" -eq 0 ]; then
    if confirm "enable+start owl-agent?"; then
      run sudo systemctl daemon-reload
      run sudo systemctl enable --now owl-agent.service
    fi
  fi
fi

# ----------------------------------------------------------------------------
# 8. opencode CLI wiring (providers, agents, failover daemon)
# ----------------------------------------------------------------------------
step "opencode CLI"
if [ "$SKIP_OPENCODE" -eq 1 ]; then
  info "skipped (--skip-opencode)"
elif [ "$VERIFY_ONLY" -eq 1 ]; then
  info "verify-only: skip opencode wiring"
else
  OC_DIR="$HOME/.config/opencode"
  run mkdir -p "$OC_DIR/agents" "$HOME/.local/bin" "$HOME/.config/systemd/user"
  # provider block: point at this gateway (never overwrite existing file)
  if [ -f "$OC_DIR/opencode.json" ]; then
    info "keep existing $OC_DIR/opencode.json"
  else
    warn "no $OC_DIR/opencode.json; writing minimal x3+owl providers (keys via env)"
    if [ "$DRY" -eq 0 ]; then
      cat > "$OC_DIR/opencode.json" <<'EOF'
{
  ...
  "provider": {
    ...
    "owl": {
      "npm": "@ai-sdk/openai-compatible",
      "name": "Owl freebuff-unified (free)",
      "options": {
        "baseURL": "http://127.0.0.1:8080/v1",
        "apiKey": "{env:OWL_LOCAL_KEY}"
      }
    }
  },
  "model": "x3/mimo/mimo-v2.5",
  "small_model": "x3/mimo/mimo-v2.5"
}
EOF
    else
  ...
  info "agents + failover daemon ship with workstation-backup, not this repo"
  info "restore via: workstation-backup/install.sh (steps 1,2,8)"
fi

# ----------------------------------------------------------------------------
# 9. verify: check + units + smoke
# ----------------------------------------------------------------------------
step "verify"
